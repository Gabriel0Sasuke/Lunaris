-- Export convertido de MySQL para PostgreSQL
-- Data da conversão: 2026-03-11
...
CREATE TABLE IF NOT EXISTS usuario (
  ...
  scan_afiliada integer DEFAULT NULL,
  ativa char(5) NOT NULL DEFAULT 'true',
  CONSTRAINT usuario_email_key UNIQUE (email),
  CONSTRAINT usuario_username_key UNIQUE (username),
  CONSTRAINT usuario_google_id_key UNIQUE (google_id),
  CONSTRAINT usuario_discord_id_key UNIQUE (discord_id),
  CONSTRAINT usuario_scan_afiliada_fkey FOREIGN KEY (scan_afiliada) REFERENCES scan (id)
);
...
CREATE TABLE IF NOT EXISTS manga_tags (
  ...
  CONSTRAINT manga_tags_manga_fkey FOREIGN KEY (manga_id) REFERENCES manga (id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT manga_tags_tag_fkey FOREIGN KEY (tag_id) REFERENCES tags (id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
COMMENT ON COLUMN usuario.google_id IS 'Login com Google';
COMMENT ON COLUMN usuario.account_type IS 'Admin, Scan ou user';
COMMENT ON COLUMN usuario.xp IS 'Quantidade de XP atual que o usuario possui';
COMMENT ON COLUMN usuario.discord_id IS 'Login com Discord';
COMMENT ON COLUMN usuario.scan_afiliada IS 'em qual Scan Pertence';
COMMENT ON COLUMN usuario.ativa IS 'True ou False';

SELECT setval(pg_get_serial_sequence('manga', 'id'), COALESCE((SELECT MAX(id) FROM manga), 1), true);
SELECT setval(pg_get_serial_sequence('scan', 'id'), COALESCE((SELECT MAX(id) FROM scan), 1), true);
